...
- Provider 请求只传递语言、学习目标、结构化问题摘要和合同错误，不传递完整源码。
- 代码或学习目标命中敏感模式时阻断模型调用，确定性静态结果仍保留。
- 自动测试使用固定对象，不依赖真实模型。
- 首次响应合同错误时最多修复重试一次。
...
- 不执行输入代码。
- 不把代码拼接到 shell 命令。
- C++ syntax-only 探针在临时目录中运行；若 `prlimit` 可用，则限制为 512 MiB 地址空间、4 秒 CPU、1 MiB 文件输出和 64 个文件描述符，并保留 3 秒墙钟超时。
- 真实模型调用前执行敏感模式预检。
...
- `app/application/review_service.py` 只负责流程编排，确定性评分不受 AI 输出影响。
- `app/adapters/ai_coach.py` 不把完整源码放入 provider 请求，合同失败最多重试一次。
- `app/detectors/language_detector.py` 对 g++ 缺失、超时、语法失败和特征冲突返回可观察状态。
- `app/storage/repository.py` 只保存代码哈希、评审元数据、问题快照、AI 教学结果和修订结果，不设置源码列。
- W3 测试目录共 48 项；连同 W1-W2 合同回归的统一检查共 51 项。
